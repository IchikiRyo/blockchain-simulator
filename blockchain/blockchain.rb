...
class Blockchain
  ...
  class << self
    def is_valid_chain? blockchain_to_validate
      return false if blockchain_to_validate.blocks[0] != Blockchain.get_genesis_block()
      tmp_blocks = [blockchain_to_validate.blocks[0]]
      blockchain_to_validate.blocks[1..-1].each.with_index(1) do |block, i|
